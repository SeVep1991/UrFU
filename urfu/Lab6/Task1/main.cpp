...
public:
    // Конструкторы
    MyString() : data(nullptr) {} // Конструктор по умолчанию
    ...
    MyString(const MyString& other) { // Конструктор копирования
        if (other.data != nullptr) {
            data = new char[strlen(other.data) + 1]; // Выделяем память под строку и копируем в неё содержимое other.data
    ...
    MyString& operator=(const MyString& other) { // Операция присваивания
        if (this != &other) { 
            delete[] data; 
            if (other.data != nullptr) {
    ...
    bool operator==(const MyString& other) const { // Операция сравнения строк
